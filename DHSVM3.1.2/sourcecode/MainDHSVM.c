  ...
  *****************************************************************************/
  ...
  if (Options.StreamTemp) {
	...
	/*if (Options.CanopyShading) {
	  sprintf(junkfilename, "%s %s", Dump.Path, "Solar_Geometry.txt");
      if (!(junkfile = fopen(junkfilename, "wt"))) 
	    exit(1);
	}*/
  }
  ...
  while (Before(&(Time.Current), &(Time.End)) ||
	...
	if (Options.StreamTemp) {
	  ...
	  /*if (Options.CanopyShading) {
		...
		fprintf(junkfile, "%d/%d/%d-%d ", Time.Current.Month, Time.Current.Day, Time.Current.Year, Time.Current.Hour);
	    fprintf(junkfile, "%f %f\n", SolarGeo.SineSolarAltitude, SolarGeo.SolarAzimuth);
	  }*/
	}
